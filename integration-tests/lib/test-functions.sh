...
_wait_for_component_initialization() {
    ...
    while [ $attempt -le $max_attempts ]; do
      ...
      component_annotations=$(kubectl get component/"${_component_name}" -n "${tenant_namespace}" -ojson 2>/dev/null | \
...
wait_for_plrs_to_appear() {
    ...
    while [ ${#appeared_plrs[@]} -lt $count ]; do
        ...
        for component in $PTSV_COMPONENTS; do
            SHA_VAR="${component}_SHA"
            SHA="${!SHA_VAR}"
            local component_push_plr_name=$(kubectl get pr -l "pipelinesascode.tekton.dev/sha=$SHA" -n "${tenant_namespace}" --no-headers 2>/dev/null | { grep "Running" || true; } | awk '{print $1}')

            # Only update if we found a PLR and haven't already recorded this component
            if [ -n "$component_push_plr_name" ] && [ -z "${appeared_plrs["$component"]}" ]; then
                appeared_plrs["$component"]="$component_push_plr_name"
                _v="${component}_push_plr_name"
                echo "Setting $_v=$component_push_plr_name"
                declare -g "$_v=$component_push_plr_name"
            fi
        done
    done
    echo
...
# Relies on global variables: ${component}_push_plr_name, tenant_namespace, PTSV_COMPONENTS
wait_for_plrs_to_complete() {
    ...
    for component in $PTSV_COMPONENTS; do
        _v="${component}_push_plr_name"
        local component_push_plr_name="${!_v}"
    ...
    while [ ${#completed_plrs[@]} -lt $count ] || [[ " ${completed_plrs[@]} " =~ " false " ]]; do
        ...
        for component in $PTSV_COMPONENTS; do
            ...
            local completed=$(kubectl get pipelinerun "${component_push_plr_name}" -n "${tenant_namespace}" -o jsonpath='{.status.conditions[?(@.type=="Succeeded")].status}' 2>/dev/null)
            ...
            if [ -n "$completed" ]; then
              ...
              if [ "$completed" == "True" ]; then
                ...
                if [ "${retry_attempted["$component"]}" == "false" ]; then
                    ...
                    wait_for_plrs_to_appear # component_push_plr_name is set here
