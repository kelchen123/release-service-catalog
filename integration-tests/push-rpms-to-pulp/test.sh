...
patch_components_source_before_merge() {
    ...
    local template_file="push-template.yaml"
    local hello_index=
    ...
    echo "  Repository: ${component_repo_name}"
    echo "  PR Number: ${pr_number}"
    local template_file="push-template-comp2.yaml"
    local hello_index=2
...
patch_component_source_before_merge() {
  ...
  #_v="${component}_pr_number"
  #local_component_pr_number="${!_v}"
  #local _v="${component}_name"
  #local component_repo_name="${!_v}"
  _patch_tekton_templates "${component_repo_name}" "${component_pr_number}" \
    "${component_name}" "pull-request-template.yaml" "${template_file}"
  _patch_spec_file "${component_repo_name}" "${component_pr_number}" "hello${hello_index}.spec" "hello${hello_index}.spec"
